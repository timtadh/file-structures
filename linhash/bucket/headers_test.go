...
func TestBytesKVStore(t *testing.T) {
	...
	test := func(store KVStore) {
		...
		if err := check(b1, k1, v1); err != nil {
			t.Fatal(err)
		}
		if err := check(b2, k2, v2); err != nil {
			t.Fatal(err)
		}
		if err := check(b3, k3, v3); err != nil {
			t.Fatal(err)
		}
		if err := check(b4, k4, v4); err != nil {
			t.Fatal(err)
		}
		if err := check(b5, k5, v5); err != nil {
			t.Fatal(err)
		}
		...
		if err := check(b1_2, k1, v1_2); err != nil {
			t.Fatal(err)
		}
		if err := check(b2_2, k2, v2_2); err != nil {
			t.Fatal(err)
		}
		if err := check(b3_2, k3, v3_2); err != nil {
			t.Fatal(err)
		}
		if err := check(b4_2, k4, v4_2); err != nil {
			t.Fatal(err)
		}
		if err := check(b5_2, k5, v5_2); err != nil {
			t.Fatal(err)
		}
	}
